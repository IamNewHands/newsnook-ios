...
**真机结果（构建 `34-2688ad9`）**：第 1 条 ✅、第 2 条 ✅（不再「跳一下」、不再像退两层）、
第 5 条 ❌（阅读器右滑返回会**短暂闪一下空白**，比修前好但「还是不够丝滑」）→ 见下一节。

### 2026-09-26 第四轮：阅读器右滑返回闪空白

第三轮的真机反馈里最有用的信息是**对比**：设置页右滑丝滑，阅读器右滑闪空白。把四个调用方摆一起
看，形状差异只有一个——**被拖动的是哪一层**：

| 调用方 | 被拖动的那一层 | 真机结果 |
|---|---|---|
| 设置二级页（`SettingsShell`） | 不透明根层（`absolute inset-0 … bg-ink`），入场动画在内层 | 丝滑 |
...
只拖正文面有两个后果：（a）根层上下两条安全区留白（`--sat` / `--sab`）原地不动，左右又直接露出
下层列表，滑动过程中画面是「撕裂」的；（b）正文面处在一个带 `fill-mode` 终态的动画层内部，合成器
必须重画那一层才能补上让出的区域，那几帧就是「空白」。

修法是把阅读器对齐到另外三处已在真机确认丝滑的形状：根层挂 `reader-swipe-surface` + `bg-ink`、
`shellRef` 上移一层（根层成为被拖动的那一层），`reader-in` 入场动画下移到内层。

验收：

1. **不闪空白**：任一源打开正文，左缘右滑到底松手，正文应当**整块**滑出，露出的列表从第一帧
   就是列表本身，全程看不到空白。
2. **上下安全区跟着走**：滑动过程中屏幕最上/最下那两条留白应跟着正文一起右移；**原地不动**就是
   只拖了正文面，说明改动没生效。
3. **阅读器内横向滚动不受影响**：正文里的宽表格仍然能左右拖（`touch-action` 的祖先链上仍然只有
   一个 `pan-y pinch-zoom`）。
